...
def scam_typology_plot_data(df):
    ...
    total_scam_types = len(df_scam_type_interim.index)
    if total_scam_types >= 10:
        top_scam_type_list = df_scam_type_interim.iloc[0:10]['scam_type'].values.tolist()
    else:
        top_scam_type_list = df_scam_type_interim.iloc[0:total_scam_types]['scam_type'].values.tolist()
    ...
    current_scam_name = df_scam_type['scam_type'].iat[0]
    current_date = df_scam_type['date_assigned'].iat[0]
    ...
    for i in range(iteration_count):
        if df_scam_type['scam_type'].iat[i] == current_scam_name:
            ...
            while df_scam_type['date_assigned'].iat[i] != current_date:
                current_date += timedelta(days = 1)
                data_list.append(0)

            data_list.append(df_scam_type['count'].iat[i])
            current_date += timedelta(days = 1)
...
def scam_typology_plot(df):
    start_date, series_list = scam_typology_plot_data(df)
