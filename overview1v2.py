...
# from overview2_updated import d2_content
from utilsv2.d1_telco import *
from utilsv2.d1_topscamtypes import *
from utilsv2.d1_heatmap import *
from utilsv2.d1_scamtypology import *

#   Initiating Connection with DB & Getting initial time that the db is updated
...
def d1_content():
    ...
    general_style = '''
        ...
        background-color: rgba(28, 27, 66, 0.6);
    ...
    font_style = '''
        ...
        font-size: 1vw;
        font-weight: bold;
        background: linear-gradient(to right, #EDDCF7, #3ED8FF);
    ...
    #   Background
    ui.image('/media/cyberbg2.jpg').style(bg_photo)
        ...
        with ui.column().style('height: 48vh; width: 30%; flex-wrap: nowrap'):
            ...
            with ui.element('div').style('height: 10vh; width: 100%'):
                with ui.row().classes('items-center justify-center'): 
                    ...
                    ui.label('ANTI-SCAM CENTRE').style('font-family: Michroma; font-size: 1.1vw; color: #E6EAEF; letter-spacing: 0.2vw; word-spacing: 0.3vw; font-weight: bold')
                    ...
                    with ui.row().style('margin-top: 2vh'):
                        for i in range(2):
                            ...
                            if arrow == 'arrow_upward':
                                ...
                                with ui.row().classes('items-center').style(percentage_style).style('background-color: #7CFF77; border-color: #7CFF77'):
                                    with ui.element('div').style('margin: auto'):
                                        with ui.row():
                                            ui.icon(arrow).style('margin-left: 0.4vw; margin-right: -0.5vw; margin-top: 0.4vh; color: #188039; font-weight: 600')
                                            ui.label(f'{percentage_num}% {word}').style('color: #188039; letter-spacing: 0.1vw')
    ...
    #-------------------------------------------------------------------------------------------------------------------------------------------------------------#
        ...
        with ui.column().style('height: 48vh; width: 35%'):
            with ui.element('div').style('height: 100%; width: 100%').style(general_style):
                ui.label('Scam Typologies').style(font_style)
                top_scam_types_plot(df).style('width: 100%; height: 90%')
                
    #-------------------------------------------------------------------------------------------------------------------------------------------------------------#
        #   Box 4: Heatmap
        with ui.column().style('height: 48vh; width: 35%; flex-wrap: nowrap'):
            with ui.element('div').style('height: 100%; width: 100%').style(general_style):
                with ui.element('div').style('height: 7%; width: 100%'):
                    ui.label('Divisional Heatmap').style(font_style)
                with ui.element('div').style(general_style).style('height: 93%; width: 100%; overflow: hidden'):
                    heatmap(df).style('height: 100%; width: 150%; margin: auto')
                
    #-------------------------------------------------------------------------------------------------------------------------------------------------------------#

    with ui.row().style('height: 46vh; width: 100%; flex-wrap: nowrap'):
        
        #   Box 7: Scam Typology Trend
        with ui.column().style('height: 100%; width: 65%'):
            with ui.element('div').style('height: 100%; width: 100%').style(general_style):
                ui.label('Scam Typology Trend (Top 10)').style(font_style)
                scam_typology_plot(df).style('height: 90%; width: 100%')
    
    #-------------------------------------------------------------------------------------------------------------------------------------------------------------#
            
        #   Box 3: Telco-Line Termination Chart
        with ui.column().style('height: 100%; width: 35%'):
            with ui.element('div').style('height: 100%; width: 100%').style(general_style):
                ui.label('Telco Line Termination').style(font_style)
                ...
                telco_plot(telco_df).style('width: 90%; height: 100%')
